...
public class Database extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase db) {
        db.execSQL("CREATE TABLE IF NOT EXISTS Game2048(hang text primary key, cot1 TEXT, cot2 text,cot3 text,cot4 text,bestscore text)");
    }

    @Override
    public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
    }
    public String getBestScore2048(Context context)
    {
        Cursor cursor = new Database(context).getData("SELECT *FROM Game2048");
        cursor.move(1);
        return cursor.getString(5);
    }
    public void setBestScore2048(int bestScore)
    {
        SQLiteDatabase database = getWritableDatabase();
        database.execSQL("UPDATE Game2048 SET bestscore ='" +bestScore +"' WHERE hang='hang1'");
        database.close();
    }
    ...
    public void setDiemHT2048(int bestScore)
    {
        SQLiteDatabase database = getWritableDatabase();
        database.execSQL("UPDATE Game2048 SET bestscore ='" +bestScore +"' WHERE hang='hang2'");
        database.close();
    }
